...
const REPORT_FILES = (process.env.REPORT_FILES || process.env.REPORT_FILE || "public/feed-report.json,public/rangiora-feed-report.json")
  .split(",")
  .map((file) => file.trim())
  .filter(Boolean);
...
for (const reportFile of REPORT_FILES) {
  ...
  if (!Number.isFinite(generatedAt.getTime())) {
    throw new Error(`${reportFile} has no valid generated_at date.`);
  }
  if (ageHours < -1) {
    throw new Error(`${reportFile} date is unexpectedly in the future.`);
  }
  if (ageHours > MAX_AGE_HOURS) {
    throw new Error(`${reportFile} is ${ageHours.toFixed(1)} hours old; maximum is ${MAX_AGE_HOURS} hours.`);
  }
  if (!upstream || !published) {
    throw new Error(`${reportFile} contains no vehicle totals.`);
  }
  if (matchRate < MIN_MATCH_RATE) {
    throw new Error(`${reportFile} match rate is ${(matchRate * 100).toFixed(1)}%; minimum is ${(MIN_MATCH_RATE * 100).toFixed(1)}%.`);
  }
}
